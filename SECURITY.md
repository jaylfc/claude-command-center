...
- **`/api/open` sandbox** — the "open file in OS" endpoint resolves the requested path under the explicit repo/session context, command-center log directories, or CCC's pasted-image upload directories and rejects anything outside. Default action is `open -R` (reveal in Finder), not launch.
...
4. **Don't spawn subprocesses on attacker-controlled paths.** If you must, validate the path against an explicit repo/session context first (see `/api/open`).
